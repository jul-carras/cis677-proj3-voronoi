...
__global__ void cu_calc_dist(v_point *pixels_d, v_point *centers, long array_size, long centers_size)
{
	...
	if(i < array_size*array_size){
		...
		for(j = 0; j < centers_size; j++){
			x_center = centers[j].position / array_size;
			y_center = centers[j].position % array_size;	
			distance = sqrt(pow(1.0 * x_center - x_point, 2) + pow(1.0 * y_center - y_point, 2));
			
			if(j == 0){	
				shortest_dist = distance;
				closest_center.position = centers[j].position;
				closest_center.red = centers[j].red;
				closest_center.blue = centers[j].blue;
				closest_center.green = centers[j].green;
...
extern void calc_distance(v_point *pixels, v_point centers, long array_size, long centers_size)
{
	...
	cu_calc_dist <<<dimgrid, dimblock>>> (pixels_d, centers, array_size, centers_size);
